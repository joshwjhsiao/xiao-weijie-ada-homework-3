...
boot_lm <- function(d, model, conf.level = 0.95, reps = 1000)  {
  ...
  alpha <- 1 - conf.level
  df$UprCIBoot[1] <- quantile(bootstrap$beta0, 1 - (alpha/2))
  df$UprCIBoot[2] <- quantile(bootstrap$beta1, 1 - (alpha/2))
  df$LwrCIBoot[1] <- quantile(bootstrap$beta0, alpha/2)
  df$LwrCIBoot[2] <- quantile(bootstrap$beta1, alpha/2)
  df
}
```
